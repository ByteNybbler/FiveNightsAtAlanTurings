...
public class Credits : MonoBehaviour
{
    ...
    const float ALPHA_SPEED = 1f;
    const float SECONDS_PER_CHARACTER = 0.02f;
...
Ballad For You (Dynamite Headdy)",
    ...
    float alpha = 0.0f;
    // Number of seconds that the text stays at 100% opacity.
    float alphaTimer;
    int stage = 0;
    Text txt;
    int numberOfStrings;
    ...
    void Update ()
    {
        ...
        if (stage == 0)
        {
            ...
            {
                alphaTimer = strings[currentString].Length * SECONDS_PER_CHARACTER;
                stage = 1;
            }
        }
        // Stage 1: Text remains at full opacity.
        if (stage == 1)
        {
